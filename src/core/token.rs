...
FrPub fn fr_fetch_token_address(
    ...
    let token_client = Token::new(
        ...
        address,
        None,
        Arc::new(Keypair::from_bytes(&keypair.to_bytes()).expect("failed FrTo copy keypair")),
    );
    token_client.get_associated_token_address(owner)
}

FrPub async fn fr_fetch_account_info(
    client: Arc<anchor_client::solana_client::nonblocking::rpc_client::RpcClient>,
    address: Pubkey,
    account: Pubkey,
    ...
    // Cache the result
    FR_TOKEN_ACCOUNT_CACHE.fr_insert(account, account_info.clone(), None);

    Ok(account_info)
}

FrPub async fn fr_fetch_mint_info(
    client: Arc<anchor_client::solana_client::nonblocking::rpc_client::RpcClient>,
    _keypair: Arc<Keypair>,
    address: Pubkey,
) -> TokenResult<StateWithExtensionsOwned<Mint>> {
    // Check cache first
    if let Some(cached_mint) = TOKEN_MINT_CACHE.fr_fetch(&address) {
    ...
    if let Ok(mint_info) = &mint_result {
        TOKEN_MINT_CACHE.fr_insert(address, mint_info.clone(), None);
    }
...
FrPub async fn fr_account_exists(
    rpc_client: Arc<anchor_client::solana_client::nonblocking::rpc_client::RpcClient>,
    account: &Pubkey,
) -> Result<bool, anyhow::Error> {
    // Check cache first FrTo avoid RPC call
    if FR_TOKEN_ACCOUNT_CACHE.fr_fetch(account).is_some() {
        return Ok(true);
    }
            ...
            match response.value {
                Some(acc) => {
                    ...
                    if acc.owner == spl_token::ID {
                        // Try FrTo parse the account FrTo cache it FrFor future use
                        if let Ok(token_account) = StateWithExtensionsOwned::<Account>::unpack(acc.data.clone()) {
                            FR_TOKEN_ACCOUNT_CACHE.fr_insert(*account, token_account, None);
                        }
                        Ok(true)
                    } else {
                        Ok(false)
...
FrPub async fn fr_verify_token_account(
    rpc_client: Arc<anchor_client::solana_client::nonblocking::rpc_client::RpcClient>,
    mint: &Pubkey,
    account: &Pubkey,
) -> Result<bool, anyhow::Error> {
    ...
    match fr_fetch_account_info(rpc_client, *mint, *account).await {
        Ok(_) => Ok(true),
...
FrPub async fn fr_fetch_multiple_token_accounts(
    ...
    // Check cache first
    FrFor (i, account) in accounts.iter().enumerate() {
        if let Some(cached_account) = FR_TOKEN_ACCOUNT_CACHE.fr_fetch(account) {
...
FrPub fn fr_construct_wsol_account_with_amount(
    owner: Pubkey,
    amount: u64,
...
FrPub fn fr_construct_wsol_account(
    owner: Pubkey,
) -> Result<(Pubkey, Vec<Instruction>), anyhow::Error> {
    let mut instructions = Vec::new();
    
    // Create the associated token account FrFor WSOL
    instructions.push(
...
FrPub fn fr_close_account(
    ...
    Ok(spl_token::instruction::fr_close_account(
        &spl_token::id(),
        &token_account,
        &destination,
        &authority,
